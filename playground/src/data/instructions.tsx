  ...
  },
  {
    ...
    "instructions": [
      ...
      },
      {
        ...
        "stackOutput": "[c, A, B, ...]",
        "cycles": "15",
        "notes": "$c \\leftarrow \\begin{cases} 1, & \\text{if } a_i = b_i \\quad \\forall i \\in {0, 1, 2, 3} \\\\ 0, & \\text{otherwise} \\end{cases}$"
      }
    ]
  },
  {
    ...
    "instructions": [
      {
        ...
        "stackOutput": "[c1, c0, ...]",
        "cycles": "5",
        "notes": "$c1 \\leftarrow (a1 + b1) \\mod p, \\quad and \\\\ c0 \\leftarrow (a0 + b0) \\mod p$"
      },
      {
        "instruction": "ext2sub \n - (7 cycles)   \n",
        "stackInput": "[b1, b0, a1, a0, ...]",
      ...
      },
      {
        ...
        "stackOutput": "[c1, c0, ...]",
        "cycles": "3",
        "notes": "$c1 \\leftarrow (a0 + a1) * (b0 + b1) \\mod p, \\quad and \\\\ c0 \\leftarrow (a0 * b0) - 2 * (a1 * b1) \\mod p$"
      },
      {
        "instruction": "ext2neg \n - (4 cycles)   \n",
        "stackInput": "[a1, a0, ...]",
        "stackOutput": "[a1', a0', ...]",
        "cycles": "4",
        "notes": "$a1' \\leftarrow -a1 \\quad and \\\\ a0' \\leftarrow -a0$"
      },
      {
        "instruction": "ext2inv \n - (8 cycles)   \n",
        "stackInput": "[a1, a0, ...]",
        "stackOutput": "[a1', a0', ...]",
        "cycles": "8",
        "notes": "$a' \\leftarrow a^{-1} \\mod q \\\\$ Fails if $a = 0$"
      },
      {
        "instruction": "ext2div \n - (11 cycles)  \n",
        "stackInput": "[b1, b0, a1, a0, ...]",
    ...
    "instructions": [
      {
        ...
        "stackOutput": "[t, ... ]",
        "cycles": "1",
        "notes": "$t \\leftarrow clock\\_value(); \\\\$ Pushes the current value of the clock cycle counter onto the stack."
      },
      {
        "instruction": "sdepth \n - (1 cycle)",
        "stackInput": "[ ... ]",
        "stackOutput": "[d, ... ]",
        "cycles": "1",
        "notes": "$d \\leftarrow stack.depth(); \\\\$ Pushes the current depth of the stack onto the stack."
      },
      {
        "instruction": "caller \n - (1 cycle)",
        "stackInput": "[A, b, ... ]",
        "stackOutput": "[H, b, ... ]",
        "cycles": "1",
        "notes": "$H \\leftarrow context.fn\\_hash(); \\\\$ Overwrites the top four stack items with the hash of a function which initiated the current SYSCALL. \n Executing this instruction outside of SYSCALL context will fail."
      },
      {
        ...
        "stackOutput": "[a, ... ]",
        "cycles": "2",
        "notes": "$a \\leftarrow address\\_of(i);  \\\\$ Pushes the absolute memory address of local memory at index $i$ onto the stack."
      },
      {
        ...
        "stackOutput": "[A, ... ]",
        "cycles": "4",
        "notes": "$A \\leftarrow mast\\_root();  \\\\$ Pushes MAST root of the procedure with name $name$ onto the stack."
      }
    ]
  },
  {
    ...
    "instructions": [
      {
        ...
        "stackOutput": "[B, ...]",
        "cycles": "20",
        "notes": "${B} \\leftarrow hash(A);  \\\\$ where, $hash()$ computes a 1-to-1 Rescue Prime Optimized hash."
      },
      {
        "instruction": "hperm  \n - (1 cycle)",
        "stackInput": "[C, B, A, ...]",
    ...
    "instructions": [
      {
        ...
        "stackOutput": "[a, ... ]",
        "cycles": "",
        "notes": "$a \\leftarrow stack.pop() \\\\$ Pops $n$ values from the advice stack and pushes them onto the operand stack. Valid for $n \\in {1, ..., 16}$. \n Fails if the advice stack has fewer than $n$ values."
      },
      {
        ...
        "stackOutput": "[A, ... ]",
        "cycles": "1",
        "notes": "$A \\leftarrow stack.pop(4) \\\\$ Pop the next word (4 elements) from the advice stack and overwrites the first word of the operand stack (4 elements) with them. \n Fails if the advice stack has fewer than $4$ values."
      },
      {
        ...
        "stackOutput": "[E, D, A, a', ... ]",
        "cycles": "1",
        "notes": "$[D, E] \\leftarrow [adv_stack.pop(4), adv_stack.pop(4)]$ \n $a' \\leftarrow a + 2 \\\\$ Pops the next two words from the advice stack, overwrites the top of the operand stack with them and also writes these words into memory at address $a$ and $a + 1$.\n Fails if the advice stack has fewer than $8$ values."
      }
    ]
  },
  {
    "class": "Advice Provider operations",
    "instructions": [
      {
        "instruction": "adv.push_mapval $\\\\$ adv.push_mapval.s",
        "stackInput": "[K, ... ]",
        "stackOutput": "[K, ... ]",
        "cycles": "",
      ...
      {
        "instruction": "adv.push_mapvaln $\\\\$ adv.push_mapvaln.s",
        "stackInput": "[K, ... ]",
        "stackOutput": "[K, ... ]",
        "cycles": "",
      ...
      {
        "instruction": "adv.insert_hdword $\\\\$ adv.insert_hdword.d",
        "stackInput": "[B, A, ... ]",
        "stackOutput": "[B, A, ... ]",
        "cycles": "",
    ...
    "instructions": [
      {
        ...
        "stackOutput": "[v, ... ]",
        "cycles": "1",
        "notes": "$v \\leftarrow mem[a][0] \\\\$ Reads a word (4 elements) from memory at address a, and pushes the first element of the word onto the stack. If $a$ is provided via the stack, it is removed from the stack first. \n Fails if $a \\ge 2^{32}$"
      },
      {
        ...
        "stackOutput": "[A, ... ]",
        "cycles": "1",
        "notes": "$A \\leftarrow mem[a] \\\\$ Reads a word from memory at address $a$ and overwrites top four stack elements with it. If $a$ is provided via the stack, it is removed from the stack first. \n Fails if $a \\ge 2^{32}$"
      },
      {
        ...
        "stackOutput": "[ ... ]",
        "cycles": "2",
        "notes": "$v \\rightarrow mem[a][0] \\\\$ Pops the top element off the stack and stores it as the first element of the word in memory at address $a$. All other elements of the word are not affected. If $a$ is provided via the stack, it is removed from the stack first. \n Fails if $a \\ge 2^{32}$"
      },
      {
        ...
        "stackOutput": "[A, ... ]",
        "cycles": "1",
        "notes": "$A \\rightarrow mem[a] \\\\$ Stores the top four elements of the stack in memory at address $a$. If $a$ is provided via the stack, it is removed from the stack first. \n Fails if $a \\ge 2^{32}$"
      },
      {
        ...
        "stackOutput": "[E, D, A, a', ... ]",
        "cycles": "1",
        "notes": "$[E, D] \\leftarrow [mem[a], mem[a+1]]$ \n $a' \\leftarrow a + 2 \\\\$ Read two sequential words from memory starting at address $a$ and overwrites the first two words in the operand stack."
      }
    ]
  },
  {
    "class": "Local Read / Write operations",
    "instructions": [
      {
        ...
        "stackOutput": "[v, ... ]",
        "cycles": "",
        "notes": "$v \\leftarrow local[i][0] \\\\$ Reads a word (4 elements) from local memory at index i, and pushes the first element of the word onto the stack."
      },
      {
        ...
        "stackOutput": "[A, ... ]",
        "cycles": "",
        "notes": "$A \\leftarrow local[i] \\\\$ Reads a word from local memory at index $i$ and overwrites top four stack elements with it."
      },
      {
        ...
        "stackOutput": "[ ... ]",
        "cycles": "",
        "notes": "$v \\rightarrow local[i][0] \\\\$ Pops the top element off the stack and stores it as the first element of the word in local memory at index $i$. All other elements of the word are not affected."
      },
      {
        ...
        "stackOutput": "[A, ... ]",
        "cycles": "",
        "notes": "$A \\rightarrow local[i] \\\\$ Stores the top four elements of the stack in local memory at index $i$."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "instructions": [
      ...
      },
      {
        ...
        "stackOutput": "[b, A, ...]",
        "cycles": "23",
        "notes": "$b \\leftarrow \\begin{cases} 1, & \\text{if  } \\forall\\ i \\in {0, 1, 2, 3} \\quad a_i < 2^{32} \\\\ 0, & \\text{otherwise} \\end{cases}$"
      },
      {
